...
import { generateSvgCover } from './generate_covers.mjs';
...
function updateArticleCovers() {
  ...
  for (const file of files) {
    ...
    const svgPath = generateSvgCover(slug, title, category);
    
    // Update cover_image in frontmatter
    const updatedContent = content.replace(/cover_image:\s*"[^"]*"/, `cover_image: "${svgPath}"`);
    fs.writeFileSync(filePath, updatedContent, 'utf8');
  }
  console.log('[+] All published article covers generated and updated successfully!');
}
